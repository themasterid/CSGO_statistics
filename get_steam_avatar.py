...
def create_avatar(steamid):
    ...
    steamid_profile_json = f'date/{steamid}/{steamid}_profile_info_{today_date}.json'
    directory = f"{steamid}"
    ...
    if os.path.exists(f'date/{steamid}/{steamid}_profile_info_{today_date}.json'):      
        profile_data_json = open_json_file(steamid_profile_json)
    else:
        req_profile_info = requests.get(url_profile_info).json()
    ...
    if os.path.exists(f'date/{steamid}/{steamid}_avatar_{today_date}.jpg') and os.path.exists(f'date/{steamid}/{steamid}_avatarmedium_{today_date}.jpg') and os.path.exists(f'date/{steamid}/{steamid}_avatarfull_{today_date}.jpg'):
        profile_data_json = open_json_file(steamid_profile_json)
    if not(os.path.exists(f'date/{steamid}/{steamid}_avatar_{today_date}.jpg') and os.path.exists(f'date/{steamid}/{steamid}_avatarmedium_{today_date}.jpg') and os.path.exists(f'date/{steamid}/{steamid}_avatarfull_{today_date}.jpg')):
        req_profile_info = requests.get(url_profile_info).json()
        write_json_file(req_profile_info, steamid_profile_json)
        profile_data_json = open_json_file(steamid_profile_json)
        ...
        out2 = open(f"date/{steamid}/{steamid}_avatarmedium_{today_date}.jpg", "wb")
        out2.write(p2.content)
        out2.close()

        p3 = requests.get(img3)
        out3 = open(f"date/{steamid}/{steamid}_avatarfull_{today_date}.jpg", "wb")

...
# KEY = keys["key"]
KEY = "FCB90A53A4B45F80CF124D835A13A115"
KEY_STEAMID = "99999999999999999;XXXXXXXXXXXXXXXXX"
CVS = "communityvisibilitystate"

GPS = f"https://{URL_STEAM}/ISteamUser/" f"GetPlayerSummaries/v2/?key={KEY}&steamids="
...
GPB = f"https://{URL_STEAM}/ISteamUser/" f"GetPlayerBans/v1/?key={KEY}&steamids="
...
class ProfileStatus:
    ...
    def get_profile_check(self, steamid):
        steam_profile = f"date/{steamid}/{steamid}" f"_profile_info_{TODAY}.json"
    ...
    def write_json(self, date_, fname):
        with open(fname, "w", encoding=UTF8, buffering=1024**2) as file_:
            return json.dump(date_, file_, ensure_ascii=False, indent=4)

    def open_json(self, fname):
        with open(fname, "r", encoding=UTF8, buffering=1024**2) as file_:
    ...
    def check_vac_banned(self, steamid):
        file_bans = f"date/{steamid}/{steamid}" f"_ban_status_{TODAY}.json"
        ...
        try:
            open(f"{DATE}/{steamid}/{steamid}" f"_ban_status_{TODAY}.json", "r")
        except FileNotFoundError:
            request = requests.get(f"{GPB}{steamid}").json()
    ...
    def create_avatar(self, steamid):
        ...
        steamid_profile_json = f"date/{steamid}/{steamid}" f"_profile_info_{TODAY}.json"
        directory = f"{steamid}"
        ...
        try:
            ...
            if req["response"]["players"][0]["communityvisibilitystate"] == 1:
                profile_data_json = self.result.open_json(steamid_profile_json)
            elif req["response"]["players"][0]["communityvisibilitystate"] == 3:
                profile_data_json = self.result.open_json(steamid_profile_json)
        ...
        if (
            os.path.exists(f'date/{steamid}/{img1.split("/")[8]}')
            and os.path.exists(f'date/{steamid}/{img2.split("/")[8]}')
            and os.path.exists(f'date/{steamid}/{img3.split("/")[8]}')
        ):
            profile_data_json = self.open_json(steamid_profile_json)
        if not (
            os.path.exists(f'date/{steamid}/{img1.split("/")[8]}')
            ...
            with open(f"date/{steamid}/{img1.split('/')[8]}", "wb") as out1:
                out1.write(p1.content)

            p2 = requests.get(img2)
            with open(f"date/{steamid}/{img2.split('/')[8]}", "wb") as out2:
                out2.write(p2.content)

            p3 = requests.get(img3)
            with open(f"date/{steamid}/{img3.split('/')[8]}", "wb") as out3:
...
class MyWin(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.ui.textBrowser_info.setText(self.get_table_statistics(STEAMID))

        # WEAPONS
        self.ui.pushButton_update_weapons.clicked.connect(self.on_start_weapons)
        self.ui.comboBox_weapons.currentIndexChanged.connect(self.open_table_weapons)
        ...
        self.check_weapons_thread.list_all_weapons.connect(
            self.get_tables_weapons, QtCore.Qt.QueuedConnection
        )

        # FRIENDS
        self.ui.pushButton_update_friends.clicked.connect(self.on_start_friends)
        self.ui.comboBox_friends.currentIndexChanged.connect(self.open_table_friends)

        self.ui.comboBox_friends.addItems(self.get_items_combobox("all_friends"))
        self.check_friends_thread.list_all_friends.connect(
            self.get_tables_friends, QtCore.Qt.QueuedConnection
        )
        self.ui.tableWidget_friends.itemDoubleClicked.connect(self.listwidgetclicked)

        # MATCHES
        self.ui.comboBox_matches.setCurrentIndex(0)
        self.ui.comboBox_matches.currentIndexChanged.connect(self.get_info_match)
        ...
        self.ui.comboBox_bans.currentIndexChanged.connect(self.open_table_bans)
        ...
        self.ui.tableWidget_bans.itemDoubleClicked.connect(self.listwidgetclicked)
    ...
    def listwidgetclicked(self, item) -> Union[str, bool]:
        ...
        return webbrowser.open(f"https://steamcommunity.com/profiles/{item.text()}")
    ...
    def get_items_combobox(self, string_w):
        ...
        only_f = sorted(
            [f for f in listdir(path) if isfile(join(path, f))], reverse=True
        )
    ...
    def get_tables_weapons(self, list_s):
        ...
        strings = "all_weapons"
        with open(
            f"date/{strings}/{STEAMID}/{TODAY}.json", "w", encoding=UTF8
        ) as file_all:
            json.dump(list_s, file_all, ensure_ascii=False, indent=4)
    ...
    def get_info_match(self):
        ...
        matchduration = f'Время игры {date_match["match_duration"]}'
        score_ = date_match["score"]
        score = f"Счет {score_}"

        pixmap = QPixmap(IMG_MAPS.get(map_match, "img/imgs_maps/nomap.jpg"))
        ...
        for val in range(10):
            ...
            steamid_i.append(date_match["team"][val]["steamid64"])
            ...
            vac_status.append(
                self.result.check_vac_banned(steamid_i[val])["players"][0]["VACBanned"]
            )
            player_name_i.append(date_match["team"][val]["player_name"])

        self.ui.label_vac_status_1.setText("VAC BAN" if vac_status[0] else "███████")
        self.ui.label_vac_status_2.setText("VAC BAN" if vac_status[1] else "███████")
        self.ui.label_vac_status_3.setText("VAC BAN" if vac_status[2] else "███████")
        self.ui.label_vac_status_4.setText("VAC BAN" if vac_status[3] else "███████")
        self.ui.label_vac_status_5.setText("VAC BAN" if vac_status[4] else "███████")
        self.ui.label_vac_status_6.setText("VAC BAN" if vac_status[5] else "███████")
        self.ui.label_vac_status_7.setText("VAC BAN" if vac_status[6] else "███████")
        self.ui.label_vac_status_8.setText("VAC BAN" if vac_status[7] else "███████")
        self.ui.label_vac_status_9.setText("VAC BAN" if vac_status[8] else "███████")
        self.ui.label_vac_status_10.setText("VAC BAN" if vac_status[9] else "███████")
        ...
        if int(score_.split(":")[0]) < int(score_.split(":")[1]):
            score_ = f"Проиграли {score_} Выиграли"
        elif int(score_.split(":")[0]) > int(score_.split(":")[1]):
            score_ = f"Выиграли {score_} Проиграли"
        elif int(score_.split(":")[0]) == int(score_.split(":")[1]):
            score_ = f"Ничья {score_} Ничья"

        self.ui.label_csore_center.setText(score_)
        self.ui.pname1_avatar.setPixmap(QPixmap(f"date/{steamid_i[0]}/{imgs[0]}"))
        self.ui.pname2_avatar.setPixmap(QPixmap(f"date/{steamid_i[1]}/{imgs[1]}"))
        self.ui.pname3_avatar.setPixmap(QPixmap(f"date/{steamid_i[2]}/{imgs[2]}"))
        self.ui.pname4_avatar.setPixmap(QPixmap(f"date/{steamid_i[3]}/{imgs[3]}"))
        self.ui.pname5_avatar.setPixmap(QPixmap(f"date/{steamid_i[4]}/{imgs[4]}"))
        self.ui.pname6_avatar.setPixmap(QPixmap(f"date/{steamid_i[5]}/{imgs[5]}"))
        self.ui.pname7_avatar.setPixmap(QPixmap(f"date/{steamid_i[6]}/{imgs[6]}"))
        self.ui.pname8_avatar.setPixmap(QPixmap(f"date/{steamid_i[7]}/{imgs[7]}"))
        self.ui.pname9_avatar.setPixmap(QPixmap(f"date/{steamid_i[8]}/{imgs[8]}"))
        self.ui.pname10_avatar.setPixmap(QPixmap(f"date/{steamid_i[9]}/{imgs[9]}"))
    ...
    def get_info_profile(self, steamid):
        ...
        if communityvisibilitystate == 1:
            ...
            self.ui.label_avatar.setPixmap(QPixmap(f"date/{steamid}/{img}"))
            ...
            self.ui.label_avatar.setPixmap(QPixmap(f"date/{steamid}/{img}"))
            self.ui.label_personaname.setText(
                result["response"]["players"][0]["personaname"] + online_status
            )
            try:
    ...
    def get_country_info(self, steamid):
        result = self.result.get_profile_check(steamid)
        text_location = ""
        load_from_file = f"date/{steamid}/{steamid}" f"_profile_info_{TODAY}.json"
        load_location_from_file_1 = (
            f"date/{steamid}/{steamid}" f"_profile_location_1_{TODAY}.json"
        )
        self.load_location_from_file_2 = (
            f"date/{steamid}/{steamid}" f"_profile_location_2_{TODAY}.json"
        )
        self.load_location_from_file_3 = (
            f"date/{steamid}/{steamid}" f"_profile_location_3_{TODAY}.json"
        )
        ...
        try:
            ...
            self.result.write_json(request, load_from_file)
        ...
        try:
            loccountrycode = result["response"]["players"][0]["loccountrycode"]
            location_all_1 = "https://steamcommunity.com/actions/QueryLocations/"
            location_req_1 = requests.get(location_all_1).json()
            self.result.write_json(location_req_1, load_location_from_file_1)
            location_file_1 = self.result.open_json(load_location_from_file_1)
        ...
        try:
            ...
            self.locstatecode = self.req_profile["response"]["players"][0][
                "locstatecode"
            ]
            self.location_url_2 = (
            ...
            self.result.write_json(self.location_req_2, self.load_location_from_file_2)
            self.location_file_2 = self.result.open_json(self.load_location_from_file_2)
        ...
        try:
            loccityid = self.req_profile["response"]["players"][0]["loccityid"]
            self.location_url_3 = (
                "https://steamcommunity.com/actions/QueryLocations/"
                f"{loccountrycode}/{self.locstatecode}"
            )
            self.location_req_3 = requests.get(self.location_url_3).json()
            self.result.write_json(self.location_req_3, self.load_location_from_file_3)
            self.location_file_3 = self.result.open_json(self.load_location_from_file_3)
        ...
        try:
            self.locstatecode = result["response"]["players"][0]["locstatecode"]
            self.location_file_2 = self.result.open_json(self.load_location_from_file_2)
            for _ in self.location_file_2:
                if _["statecode"] == self.locstatecode:
                    text_location += _["statename"] + ", "
        ...
        try:
            loccityid = result["response"]["players"][0]["loccityid"]
            self.location_file_3 = self.result.open_json(self.load_location_from_file_3)
            for _ in self.location_file_3:
                if _["cityid"] == loccityid:
                    text_location += _["cityname"]
    ...
    def get_table_statistics(self, steamid):
        ...
        # self.result.create_avatar(steamid)
        self.get_info_profile(steamid)
        profile_info = f"date/{steamid}/{steamid}" f"_profile_info_{TODAY}.json"
        tta = ""
        ...
        if communityvisibilitystate == 1:
            ...
            self.ui.label_avatar.setPixmap(QPixmap(f"date/{steamid}/{img}"))
            self.ui.label_personaname.setText(
                result["response"]["players"][0]["personaname"] + " (Приватный профиль)"
            )
            return NO_INFO_USERS
        ...
        online_status = status_p[personastate]
        tta += 'SteamID - ' f'{result["response"]["players"][0]["steamid"]}\n'
        ...
        try:
            tta += (
                'Реальное имя -' f'{result["response"]["players"][0]["realname"]}' '\n'
            )
        except KeyError:
            tta += "Реальное имя - ██████████ \n"
        tmp_timec = datetime.fromtimestamp(
            result["response"]["players"][0]["timecreated"]
        )
        tta += f"Дата создания профиля - {tmp_timec}\n"
        tta += f"Страна - {self.ui.label_loccountrycode.text()}\n"
    ...
    def open_new_profile(self):
        ...
        if steam_id == "":
            self.statusBar().showMessage("Enter steam id or str")
        ...
        self.ui.textBrowser_info.setText(self.get_table_statistics(steam_id))
    ...
    def on_change_wp_rows(self, info_progress_bar_w, wstats):
        ...
        self.ui.progressBar_bans.setProperty("value", self.info_progress_bar_w)
...
class CheckWeaponsThread(QtCore.QThread, MyWin):
    ...
    def get_info_weapons(self, steamid):
        ...
        total_ksh_ak47 = tuple(self.get_wkey(item, steamid) for item in ak47)

        aug = (f"{tl_ks_}aug", f"{tl_ss_}aug", f"{tl_hs_}aug")
        total_ksh_aug = tuple(self.get_wkey(item, steamid) for item in aug)

        awp = (f"{tl_ks_}awp", f"{tl_ss_}awp", f"{tl_hs_}awp")
        total_ksh_awp = tuple(self.get_wkey(item, steamid) for item in awp)

        deagle = (f"{tl_ks_}deagle", f"{tl_ss_}deagle", f"{tl_hs_}deagle")
        total_ksh_deagle = tuple(self.get_wkey(item, steamid) for item in deagle)

        elite = (f"{tl_ks_}elite", f"{tl_ss_}elite", f"{tl_hs_}elite")
        total_ksh_elite = tuple(self.get_wkey(item, steamid) for item in elite)

        famas = (f"{tl_ks_}famas", f"{tl_ss_}famas", f"{tl_hs_}famas")
        total_ksh_famas = tuple(self.get_wkey(item, steamid) for item in famas)

        fiveseven = (f"{tl_ks_}fiveseven", f"{tl_ss_}fiveseven", f"{tl_hs_}fiveseven")
        total_ksh_fiveseven = tuple(self.get_wkey(item, steamid) for item in fiveseven)

        g3sg1 = (f"{tl_ks_}g3sg1", f"{tl_ss_}g3sg1", f"{tl_hs_}g3sg1")
        total_ksh_g3sg1 = tuple(self.get_wkey(item, steamid) for item in g3sg1)

        galilar = (f"{tl_ks_}galilar", f"{tl_ss_}galilar", f"{tl_hs_}galilar")
        total_ksh_galilar = tuple(self.get_wkey(item, steamid) for item in galilar)

        glock = (f"{tl_ks_}glock", f"{tl_ss_}glock", f"{tl_hs_}glock")
        total_ksh_glock = tuple(self.get_wkey(item, steamid) for item in glock)

        m249 = (f"{tl_ks_}m249", f"{tl_ss_}m249", f"{tl_hs_}m249")
        total_ksh_m249 = tuple(self.get_wkey(item, steamid) for item in m249)

        m4a1 = (f"{tl_ks_}m4a1", f"{tl_ss_}m4a1", f"{tl_hs_}m4a1")
        total_ksh_m4a1 = tuple(self.get_wkey(item, steamid) for item in m4a1)

        mac10 = (f"{tl_ks_}mac10", f"{tl_ss_}mac10", f"{tl_hs_}mac10")
        total_ksh_mac10 = tuple(self.get_wkey(item, steamid) for item in mac10)

        mag7 = (f"{tl_ks_}mag7", f"{tl_ss_}mag7", f"{tl_hs_}mag7")
        total_ksh_mag7 = tuple(self.get_wkey(item, steamid) for item in mag7)

        mp7 = (f"{tl_ks_}mp7", f"{tl_ss_}mp7", f"{tl_hs_}mp7")
        total_ksh_mp7 = tuple(self.get_wkey(item, steamid) for item in mp7)

        mp9 = (f"{tl_ks_}mp9", f"{tl_ss_}mp9", f"{tl_hs_}mp9")
        total_ksh_mp9 = tuple(self.get_wkey(item, steamid) for item in mp9)

        negev = (f"{tl_ks_}negev", f"{tl_ss_}negev", f"{tl_hs_}negev")
        total_ksh_negev = tuple(self.get_wkey(item, steamid) for item in negev)

        nova = (f"{tl_ks_}nova", f"{tl_ss_}nova", f"{tl_hs_}nova")
        total_ksh_nova = tuple(self.get_wkey(item, steamid) for item in nova)

        hkp2000 = (f"{tl_ks_}hkp2000", f"{tl_ss_}hkp2000", f"{tl_hs_}hkp2000")
        total_ksh_hkp2000 = tuple(self.get_wkey(item, steamid) for item in hkp2000)

        p250 = (f"{tl_ks_}p250", f"{tl_ss_}p250", f"{tl_hs_}p250")
        total_ksh_p250 = tuple(self.get_wkey(item, steamid) for item in p250)

        p90 = (f"{tl_ks_}p90", f"{tl_ss_}p90", f"{tl_hs_}p90")
        total_ksh_p90 = tuple(self.get_wkey(item, steamid) for item in p90)

        bizon = (f"{tl_ks_}bizon", f"{tl_ss_}bizon", f"{tl_hs_}bizon")
        total_ksh_bizon = tuple(self.get_wkey(item, steamid) for item in bizon)

        sawedoff = (f"{tl_ks_}sawedoff", f"{tl_ss_}sawedoff", f"{tl_hs_}sawedoff")
        total_ksh_sawedoff = tuple(self.get_wkey(item, steamid) for item in sawedoff)

        scar20 = (f"{tl_ks_}scar20", f"{tl_ss_}scar20", f"{tl_hs_}scar20")
        total_ksh_scar20 = tuple(self.get_wkey(item, steamid) for item in scar20)

        sg556 = (f"{tl_ks_}sg556", f"{tl_ss_}sg556", f"{tl_hs_}sg556")
        total_ksh_sg556 = tuple(self.get_wkey(item, steamid) for item in sg556)

        ssg08 = (f"{tl_ks_}ssg08", f"{tl_ss_}ssg08", f"{tl_hs_}ssg08")
        total_ksh_ssg08 = tuple(self.get_wkey(item, steamid) for item in ssg08)

        tec9 = (f"{tl_ks_}tec9", f"{tl_ss_}tec9", f"{tl_hs_}tec9")
        total_ksh_tec9 = tuple(self.get_wkey(item, steamid) for item in tec9)

        ump45 = (f"{tl_ks_}ump45", f"{tl_ss_}ump45", f"{tl_hs_}ump45")
        total_ksh_ump45 = tuple(self.get_wkey(item, steamid) for item in ump45)

        xm1014 = (f"{tl_ks_}xm1014", f"{tl_ss_}xm1014", f"{tl_hs_}xm1014")
        total_ksh_xm1014 = tuple(self.get_wkey(item, steamid) for item in xm1014)
        ...
        return (
            ...
            (
                ...
                str(round(total_ksh_fiveseven[0] / total_sum * 100, 2)) + "%",
            ),
            (
                "G3SG1",
    ...
    def get_wkey(self, finded, steamid):
        get_weapons = f"date/{steamid}/{steamid}" f"_all_statistic_{TODAY}.json"

        try:
            open(get_weapons, "r", encoding=UTF8)
        except FileNotFoundError:
            self.result.write_json(
                requests.get(f"{GUSFG}{steamid}").json(), get_weapons
            )
...
class CheckFriendsThread(QtCore.QThread, MyWin, ProfileStatus):
    ...
    def run(self):
        ...
        try:
            ...
            self.result.write_json(requests.get(GSF).json(), all_fr_jsn)

        fr_steam = self.result.open_json(all_fr_jsn)
        friend_l = fr_steam["friendslist"]["friends"]
        for i in range(len(friend_l)):
            s_id_friend = fr_steam["friendslist"]["friends"][i]["steamid"]
            friend_since_friend = str(
                datetime.fromtimestamp(
                    fr_steam["friendslist"]["friends"][i]["friend_since"]
                )
            ).split(" ")[0]
            ...
            try:
                ...
                req_friends = requests.get(f"{GPS}{s_id_friend}").json()
                friend_steamid_json = (
                    f'date/{STEAMID}/'
                    f'{req_friends["response"]["players"][0]["steamid"]}'
                    '.json'
                )
                self.result.write_json(req_friends, friend_steamid_json)
            ...
            friend_steamid_json = f"date/{STEAMID}/{s_id_friend}.json"
...
class CheckVacThread(QtCore.QThread, MyWin):
    ...
    def run(self):
        ...
        URL_GPS = f"https://{URL_STEAM}/" "ISteamUser/GetPlayerSummaries/v2/"
        ...
        while self.running:
            ...
            for count, player in enumerate(results_bans):
                ...
                if (
                    player.VACBanned
                    or player.NumberOfGameBans > 0
                    or player.EconomyBan != "none"
                ):
                    tmp_users.append(
                        [
                            player.SteamId,
                            ...
                            ("BAN" if player.EconomyBan != "none" else ""),
                        ]
                    )

            self.list_all_users.emit(
                tmp_users,
            )
            self.running = False
...
def get_api_info(ids, id_resolve_url, api_key):
    return DotMap(requests.get(id_resolve_url.format(api_key, ",".join(ids))).json())
